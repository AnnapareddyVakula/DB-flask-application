...
db_config = {
    'user': 'vakulauser',
    'password': 'vakulauser',
    'host': '34.135.30.73',  # Replace with your MySQL IP address
    'database': 'studentdb',
}

# GCP Bucket Name
bucket_name = 'pythonflask-bucket'

# Set Google Cloud credentials environment variable
#os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = r'C:\Users\vakul\Desktop\SEM 3\distributed systems\flaskassignment\backend\vakula-pythonflask-600d058d662b.json'
...
def insert_data():
    ...
    connection = mysql.connector.connect(**db_config)
    cursor = connection.cursor()
    query = "INSERT INTO student_registration (name, registration_number) VALUES (%s, %s)"
    cursor.execute(query, (name, reg_number))
    connection.commit()
    
    # Fetch inserted data
    cursor.execute("SELECT name, registration_number FROM student_registration WHERE name = %s AND registration_number = %s", (name, reg_number))
    inserted_data = cursor.fetchone()
...
def fetch_data():
    connection = mysql.connector.connect(**db_config)
    cursor = connection.cursor()
    query = "SELECT name, registration_number FROM student_registration ORDER BY id DESC LIMIT 3"
    cursor.execute(query)
    records = cursor.fetchall()
    cursor.close()
    connection.close()
